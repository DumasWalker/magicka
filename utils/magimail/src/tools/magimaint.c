...
#ifdef MSGBASE_SQ3
bool ProcessAreaSQ3(struct Area *area,bool maint, bool pack, bool verbose);
#endif
...
bool ProcessAreaMSG(struct Area *area,bool maint, bool pack, bool verbose)
{
   ...
   {
      strcpy(StoredMsg.From,"MagiMail");
...
bool ProcessAreaSQ3(struct Area *area, bool maint, bool pack, bool verbose)
{
   ...
   static const char *deletenum = "DELETE FROM msgs WHERE id IN (SELECT id FROM msgs ORDER BY id LIMIT ?)";
   static const char *deletedays = "DELETE FROM msgs WHERE id IN (SELECT id FROM msgs WHERE date < ? ORDER BY id)";
   ...
   if (!open_sq3_database(area->Path, &db)) {
      printf("Failed to open message database \"%s\"\n", area->Path);
      return TRUE;
   }
   ...
   if (maint && area->KeepNum != 0) {
      if (total > area->KeepNum) {
         if (sqlite3_prepare_v2(db, deletenum, strlen(deletenum), &res, NULL) != SQLITE_OK) {
            printf("Error preparing sql\n");
            sqlite3_close(db);
            return TRUE;
         }

         sqlite3_bind_int(res, 1, total - area->KeepNum);

         if (sqlite3_step(res) != SQLITE_DONE) {
            printf("Error triming message base\n");
            sqlite3_finalize(res);
            sqlite3_close(db);
            return TRUE;
         }
         ...
         sqlite3_finalize(res);
         printf(" deleted #%u messages\n", total - area->KeepNum);
      }
   }

   if (maint && area->KeepDays != 0) {
      today=(time(NULL)-sq3_utcoffset) / (24*60*60);
      cutoff = (today - area->KeepDays) * (24 * 60 * 60);
      if (sqlite3_prepare_v2(db, deletedays,strlen(deletedays), &res, NULL) != SQLITE_OK) {
         printf("Error preparing sql\n");
         sqlite3_close(db);
         return TRUE;
      }
      sqlite3_bind_int(res, 1, cutoff);

      if (sqlite3_step(res) != SQLITE_DONE) {
         printf("Error deleting messages\n");
         sqlite3_finalize(res);
         sqlite3_close(db);
         return TRUE;
      }

      printf(" deleted old messages\n");
   ...
   sqlite3_close(db);
   if (pack) {
      printf("Pack unnessisary for SQ3 messagebases\n");
   }
         ...
         if(res == 0)
			{
			   ...
			   if(today-day > area->KeepDays && !(Header_S.Attribute & JAM_MSG_DELETED))
				{

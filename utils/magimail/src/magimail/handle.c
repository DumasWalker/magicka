...
char *StripRe(char *str)
{
   ...
   for (;;)
   {
      if(strnicmp (ptr, "Re:", 3)==0)
      {
         ptr += 3;
         if (*ptr == ' ') ptr++;
      }
      else if(strnicmp (ptr, "Re^", 3)==0 && ptr[4]==':')
      {
         ptr += 5;
         if (*ptr == ' ') ptr++;
      }
      else if(strnicmp (ptr, "Re[", 3)==0 && ptr[4]==']' && ptr[5]==':')
      {
         ptr += 6;
         if (*ptr == ' ') ptr++;
      }
...
bool HandleEchomail(struct MemMessage *mm)
{
   ...
   if(!(mm->Flags & MMFLAG_EXPORTED) && temparea->Messagebase)
   {
      ...
      if(config.cfg_Flags & CFG_STRIPRE) {
         char *subjstr = strdup(StripRe(mm->Subject));
         strcpy(mm->Subject, subjstr);
         free(subjstr);
      }
   ...
   for(tmp=(struct TextChunk *)mm->TextChunks.First;tmp;tmp=tmp->Next)
   {
      ...
      while(c<tmp->Length)
      {
         ...
         if(strncmp(&tmp->Data[c],"\x01Via",4)==0)
         {
            ...
            if(strstr(via,"MagiMail"))
            {
               /* Is created by MagiMail */
...
bool Bounce(struct MemMessage *mm,char *reason,bool headeronly)
{
   ...
   {
      LogWrite(1,TOSSINGERR,"No bounce messages sent for messages created by MagiMail");
      return(TRUE);
   }
...
bool HandleNetmail(struct MemMessage *mm)
{
   ...
   else
   {
      ...
      sprintf(buf,"\x01Via %s @%04u%02u%02u.%02u%02u%02u MagiMail/" OS_PLATFORM_NAME " " VERSION "\x0d",
         buf2,
         tp->tm_year+1900,
         tp->tm_mon+1,
         tp->tm_mday,
